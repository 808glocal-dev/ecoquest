// activity_analogy_patch.js - 내활동 탭 개인 CO2 비유
(function(){
  function getPersonalAnalogy(kg){
    ...
    if(kg < 1)     return {emoji:'🌱', text:`${(kg*1000).toFixed(0)}g 절감!`, sub:`☕ 일회용컵 ${Math.floor(kg/0.332)||1}개 안 쓴 효과예요`};
    ...
    if(kg < 3.5)   return {emoji:'🌿', text:`${kg.toFixed(1)}kg 절감! 새싹이 나요`, sub:`💡 전기 ${Math.floor(kg/0.46)}kWh 절약 효과예요`};
    ...
    if(kg < 6.5)   return {emoji:'🌿', text:`${kg.toFixed(1)}kg 절감! 대단해요`, sub:`💧 샤워 5분 단축 ${Math.floor(kg/0.38)}회 효과예요`};
    ...
    if(kg < 9.5)   return {emoji:'🌿', text:`${kg.toFixed(1)}kg 절감! 멋져요`, sub:`💡 전기 ${Math.floor(kg/0.46)}kWh 절약 효과예요 (하루치)`};
    ...
    if(kg < 19)    return {emoji:'🌳', text:`${kg.toFixed(1)}kg 절감!`, sub:`💡 전기 ${Math.floor(kg/0.46)}kWh 절약 효과예요`};
    ...
    if(kg < 45)    return {emoji:'🌳', text:`${kg.toFixed(0)}kg 절감! 에코 챔피언`, sub:`💡 가정 전기 ${Math.floor(kg/0.46)}kWh 절약 효과예요`};
    ...
    if(kg < 70)    return {emoji:'🌲', text:`나무 ${(kg/21.4).toFixed(0)}그루!`, sub:`💧 샤워 5분 단축 ${Math.floor(kg/0.38)}회 효과예요`};
    if(kg < 80)    return {emoji:'🌲', text:`${kg.toFixed(0)}kg 절감! 숲이 자라요`, sub:`🚗 자동차로 ${Math.floor(kg/0.21)}km 덜 달린 효과예요`};
    if(kg < 90)    return {emoji:'🌲', text:`나무 ${(kg/21.4).toFixed(0)}그루!`, sub:`🏠 가정 한 달 전기 배출량 수준이에요`};
    ...
    if(kg < 200)   return {emoji:'🌲', text:`${kg.toFixed(0)}kg! 대단한 숲`, sub:`🏠 한국 가구 한 달 전기 배출량이에요`};
