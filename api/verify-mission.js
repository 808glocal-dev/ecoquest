...
★ 이 미션의 목적은 대기전력 차단이에요. 아래 기준으로 엄격하게 판정하세요.

passed: true 조건 (둘 중 하나면 통과):
- 멀티탭 전원 스위치가 꺼진 상태 (LED 불빛 없음, 스위치 OFF)
- 콘센트에서 플러그가 완전히 빠진 상태 (전기 연결 차단)

passed: false 조건:
- 멀티탭 LED가 켜진 상태 (빨간불/초록불 등 불빛 있음)
- 플러그가 콘센트에 꽂혀 있는 상태
- 멀티탭/콘센트/플러그가 사진에 없음
- 꺼짐/뽑힘 상태가 불분명 → false
