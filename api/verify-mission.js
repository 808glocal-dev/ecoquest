...
1. 멀티탭 메인 전원 스위치가 OFF 상태 (LED 불빛 없음)
2. 개별 콘센트 스위치가 꺼진 상태
3. 콘센트에서 플러그가 빠진 상태
4. 멀티탭/콘센트가 사진에 보이고 LED 불빛이 없음
5. 스위치 상태가 불분명하지만 멀티탭/콘센트가 보임 → 통과

passed: false 조건 (아래 경우만):
- 멀티탭 LED가 명확하게 켜진 상태 (빨간불/초록불이 확실히 보임)
- 멀티탭/콘센트가 사진에 전혀 없음

판단이 조금이라도 애매하면 무조건 passed: true로 판정하세요.
