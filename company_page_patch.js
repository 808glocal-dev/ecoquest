/* =====================================================
   EcoQuest – 기업 탭 독립 페이지 분리 패치
   기존 corporate_patch.js의 탭 버튼 부분을 교체
   ===================================================== */
(function () {
  'use strict';
  ...
  function injectCompanyPage() {
    ...
    page.innerHTML = `
      <div style="background:linear-gradient(135deg,#1a6b3a,#2ECC71);padding:16px;color:#fff">
        <div style="font-size:18px;font-weight:900;margin-bottom:2px">🏢 기업/단체</div>
        <div style="font-size:12px;color:rgba(255,255,255,.75)">소속 기업 등록 및 임직원 현황</div>
      </div>
      ...
      <div style="padding:12px 12px 0">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
          ...
          <button onclick="loadCompanyPage()"
            style="font-size:12px;color:var(--sub);background:none;border:none;cursor:pointer;font-family:inherit">
            새로고침
          </button>
        </div>
        <div id="companyPageBox" style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--bdr);min-height:60px;margin-bottom:12px">
      ...
      <div style="padding:0 12px">
        ...
        <div id="companyRankPage" style="margin-bottom:12px">
          <div style="text-align:center;color:var(--sub);font-size:12px;padding:16px">로딩 중...</div>
        </div>
      </div>

      <!-- 임직원 미션 현황 (소속 기업 있을 때만) -->
      <div id="companyMissionSec" style="padding:0 12px;display:none">
        <div style="font-size:15px;font-weight:900;color:var(--txt);margin-bottom:8px">📊 임직원 미션 현황</div>
        <div id="companyMissionStats" style="background:#fff;border-radius:14px;padding:14px;border:1px solid var(--bdr);margin-bottom:20px">
          <div style="text-align:center;color:var(--sub);font-size:12px;padding:8px">로딩 중...</div>
        </div>
      </div>
    `;
    ...
    if (tabBar) {
      tabBar.parentElement.insertBefore(page, tabBar);
    }
  }

  // ── 2. 탭 버튼을 기업 페이지로 연결 ──
  function fixCompanyTabBtn() {
    // 기존 tb-company 버튼 제거 후 재생성
      ...
      box.innerHTML = `
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          ...
          <div style="display:flex;gap:8px">
            <input id="coCodeInpPage" class="inp" placeholder="초대 코드 입력"
      ...
      el.innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px">
          ...
          <div style="background:#f0fbf4;border-radius:12px;padding:12px;text-align:center;border:1px solid var(--bdr)">
            ...
            <div style="font-size:11px;color:var(--sub);margin-top:2px">총 미션 완료</div>
          </div>
          <div style="background:#f0fbf4;border-radius:12px;padding:12px;text-align:center;border:1px solid var(--bdr)">
      ...
      if (!coStats.length) {
        el.innerHTML = `
          <div style="background:#fff;border-radius:14px;padding:20px;text-align:center;border:1px solid var(--bdr)">
            ...
            <div style="font-size:12px;color:var(--sub)">기업을 등록하고 임직원과 함께 미션에 참여해보세요!</div>
          </div>`;
        return;
      }

      el.innerHTML = coStats.map((s, i) => `
        <div style="display:flex;align-items:center;gap:10px;padding:12px;background:#fff;
                    border-radius:12px;margin-bottom:6px;border:1px solid var(--bdr)">
          <div style="font-size:20px">${medals[i]}</div>
          <div style="font-size:28px">${s.co.emoji || '🏢'}</div>
          <div style="flex:1">
