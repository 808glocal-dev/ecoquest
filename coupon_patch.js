/* =====================================================
   EcoQuest 쿠폰 패치 v2
   - 유저 사용하기 버튼
   - 사용 시 브랜드 이메일 자동 발송 (EmailJS)
   ===================================================== */

// ── EmailJS 설정 (여기만 채우면 됨) ──
...
async function sendCouponUsedEmail(params) {
  if (EMAILJS_PUBLIC_KEY === 'YOUR_PUBLIC_KEY') {
    console.log('📧 [EmailJS 미설정] 발송 예정:', params);
    return;
  }
  try {
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      ...
      discount:      params.discount + '원',
      used_at:       new Date().toLocaleString('ko-KR'),
      user_nickname: params.userNickname || '익명',
    });
  } catch (e) { console.log('📧 발송 실패:', e); }
}
  ...
  ov.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:24px;width:100%;max-width:340px;text-align:center">
      ...
      <div style="background:#f0fbf4;border-radius:12px;padding:16px;margin-bottom:14px;border:1.5px solid var(--g1)">
        ...
        ${c.minPurchase ? `<div style="font-size:11px;color:var(--sub);margin-top:6px">${c.minPurchase.toLocaleString()}원 이상 구매 시 사용</div>` : ''}
      </div>
...
window.confirmUseCoupon = async function (docId) {
  ...
  try {
    ...
    toast('✅ 쿠폰 사용 완료!');
    if (c.brandEmail) {
      await sendCouponUsedEmail({
        brandEmail: c.brandEmail, brandName: c.brandName,
        code: c.code, discount: c.discount,
        userNickname: window.UDATA?.nickname || '익명',
      });
    }
    ...
    panel.innerHTML = `
      <div style="background:#fff;border-radius:14px;padding:14px;margin-bottom:12px;border:1px solid #eee">
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          ...
          <input id="cpEmail" placeholder="브랜드 이메일 (사용 알림 수신)" style="border:1.5px solid #ddd;border-radius:10px;padding:8px 12px;font-size:13px;font-family:inherit"/>
...
async function loadAdminCoupons() {
  const w=document.getElementById('adminCouponList'); if(!w) return;
  try {
    const snap=await window.FB.getDocs(window.FB.collection(window.FB.db,'coupons'));
    ...
    w.innerHTML=coupons.map(c=>{
      const remaining=(c.totalQty||0)-(c.usedQty||0);
      return `<div style="padding:10px;background:#f8fdf9;border-radius:10px;margin-bottom:8px;border:1px solid var(--bdr)">
...
window.addAdminCoupon=async function(){
  ...
  try {
    await window.FB.addDoc(window.FB.collection(window.FB.db,'coupons'),{
      brandName:brand,brandEmoji:emoji,brandEmail:email,discount,minPurchase,
      pointCost,totalQty,usedQty:0,codePrefix,active:true,createdAt:window.FB.serverTimestamp(),
    });
    toast('✅ 쿠폰 등록 완료!');
