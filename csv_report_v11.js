// csv_report_v11.js   (구 csv_detail_patch.js / csv_report_v10.js 대체)
// ─────────────────────────────────────────────────────────────
// v10 → v11
...
(function(){
  ...
  async function renderPanel(){
    ...
    const btns = D.companyNames.map(c => {
      const M = D.coMonths[c];
      const recs = Object.values(D.byCo).filter(x=>x.c===c);
      let v=0; const u=new Set();
      recs.forEach(x=>{ v+=x.v; x.u.forEach(k=>u.add(k)); });
      return '<button onclick="exportCompanyCSV(' + JSON.stringify(c).replace(/"/g,'&quot;') + ')" ' +
    ...
    box.innerHTML = shell(
      '<div style="display:flex;gap:6px;margin-bottom:8px">' +
        '<button onclick="exportOverallCSV()" style="' + BS + 'background:#1a1a2e;color:#fff;flex:1">📊 전체 요약</button>' +
        '<button onclick="exportAllCSV()" style="' + BS + 'background:#F39C12;color:#fff;flex:1">📦 전부 받기</button>' +
      '</div>' +
  ...
  function shell(inner){
    return '<div style="background:#fff;border-radius:14px;padding:14px;border:1px solid #eee;margin-bottom:10px">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">' +
        ...
        '<button onclick="eqDlReload()" style="background:#f0f0f0;border:none;border-radius:8px;padding:5px 10px;' +
  ...
  console.log('%c[csv v11] 버튼 패널 + 미션 상세 준비됨','color:#fff;background:#1a1a2e;padding:4px 8px;border-radius:4px;font-weight:bold');
})();
